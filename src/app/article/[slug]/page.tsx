...
import { mockArticles, services } from "@/data/mock";
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 lg:gap-16">
          ...
          <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.6, delay: 0.1 }}>
            ...
            <p className={`text-sm font-medium uppercase tracking-wider mb-2 ${accent}`}>{article.category_fr}</p>
            ...
            <div className="flex items-center gap-4 mb-6">
              ...
              <div className="flex items-center gap-3 bg-charcoal-soft rounded-full px-4 py-2">
                ...
                <button
                  onClick={() => setQuantity(Math.min(article.stock_available, quantity + 1))}
                  className="w-7 h-7 rounded-full border border-gold/30 flex items-center justify-center text-gold hover:bg-gold hover:text-charcoal-deep transition-all"
                >
          ...
          <div className="mt-20">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              {relatedArticles.map((related) => (
                <Link key={related.id} href={`/article/${related.slug}`} className={`group glass-card rounded-xl overflow-hidden border border-transparent ${serviceAccent[related.service] ? `hover:border-current` : ""} transition-all`}>
                  <div className="relative h-36 overflow-hidden">
                    <Image src={related.images[0]} alt={related.name_fr} fill className="object-cover group-hover:scale-110 transition-transform duration-500" sizes="25vw" />
                  </div>
                  <div className="p-3">
                    <p className={`text-xs font-medium line-clamp-2 group-hover:${serviceAccent[related.service]} transition-colors text-off-white`}>{related.name_fr}</p>
                    <p className={`text-sm font-bold mt-1 ${serviceAccent[related.service]}`}>{formatPrice(related.price_per_day)}</p>
                  </div>
                </Link>
              ))}
            </div>
